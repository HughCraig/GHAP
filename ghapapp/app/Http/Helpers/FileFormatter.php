...
class FileFormatter 
{
    ...
    public static function toGeoJSON ($results, $sources) { 
        ...
        foreach($results as $r) {
            ...
            $proppairs = array();
	    	if (!empty($r->title)){$proppairs["name"] = $r->title;} else {$proppairs["name"] = $r->placename;}
	    	if (!empty($r->placename)){$proppairs["placename"] = $r->placename;}
            
			if (!empty($r->description)){$proppairs["description"] = $r->description;}
            if (!empty($r->anps_id)){$id = "a".base_convert($r->anps_id,10,16);}
            if (!empty($r->dataitem_id)){$id = "t".base_convert($r->dataitem_id,10,16);}
            $proppairs["id"] = $id;
			if (!empty($r->warning)){$proppairs["warning"] = $r->warning;}
			if (!empty($r->state_code)){$proppairs["state"] = $r->state_code;}
			if (!empty($r->parish)){$proppairs["parish"] = $r->parish;}
			if (!empty($r->feature_term)){$proppairs["feature_term"] = $r->feature_term;}
			if (!empty($r->lga)){$proppairs["lga"] = $r->lga;}
			if (!empty($r->source)){$proppairs["source"] = $r->source;}
            //
            if (!empty($sources[$r->anps_id])){$proppairs["anps_sources"] = $sources[$r->anps_id];}
            if (!empty($r->source)){$proppairs["original_data_source"] = $r->source;}
            if (!empty($r->original_data_source)){$proppairs["original_data_source"] = $r->original_data_source;}


			if (!empty($contribution)){$proppairs["contribution"] = $contribution;}
			if (!empty($r->datestart)){$proppairs["datestart"] = $r->datestart;} elseif (!empty($r->tlcm_start)){$proppairs["datestart"] = $r->tlcm_start;}
			if (!empty($r->dateend)){$proppairs["dateend"] = $r->dateend;} elseif (!empty($r->tlcm_end)){$proppairs["dateend"] = $r->tlcm_end;}

            if (!empty($r->datestart)){$proppairs["latitude"] = $r->latitude;} elseif (!empty($r->tlcm_latitude)){$proppairs["latitude"] = $r->tlcm_latitude;}
			if (!empty($r->dateend)){$proppairs["longitude"] = $r->longitude;} elseif (!empty($r->tlcm_longitude)){$proppairs["longitude"] = $r->tlcm_longitude;}
            
			if (!empty($r->external_url)){$proppairs["linkback"] = $r->external_url;}
