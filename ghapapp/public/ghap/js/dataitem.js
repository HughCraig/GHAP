...
$("#edit_dataset_form, #new_dataset_form").on('submit', function() {
    ...
    if (temporalfrom.value && !dateMatchesRegex(temporalfrom.value)) {temporalfrom.classList.add('is-invalid'); alert('Temporal Coverage "From" field is NOT in a valid format!'); return false;}
    if (temporalto.value && !dateMatchesRegex(temporalto.value)) {temporalto.classList.add('is-invalid'); alert('Temporal Coverage "To" field is NOT in a valid format!'); return false;}
});
 ...
 $("main").on('click','[name="edit_dataitem_button"]', function() { 
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            $('#row_id_'+id+' #datestart').attr({ 'oldvalue': new_start, 'value': new_start, 'text': new_start }).prop({'value': new_start});
            $('#row_id_'+id+' #dateend').attr({ 'oldvalue': new_end, 'value': new_end, 'text': new_end }).prop({'value': new_end});
