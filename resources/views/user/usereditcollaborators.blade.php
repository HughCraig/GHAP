   ...
   <div class="row">
        <div class="col-xs-3">
            <table class="table-sm table-bordered">
                <tr><td>Name</td><td>{{$ds->name}}</td></tr>
                <tr style="height: 50px; overflow: auto"><td>Description</td><td>{{$ds->description}}</td></tr>
                <tr><td>Role</td><td id="dsrole">{{$ds->pivot->dsrole}}</td></tr>
                <tr><td>Entries</td><td id="dscount">{{count($ds->dataitems)}}</td></tr>
                <tr><td>Created</td><td>{{$ds->created_at}}</td></tr>
                <tr><td>Updated</td><td id="dsupdatedat">{{$ds->updated_at}}</td></tr>
    ...
    <table id="collabtable" class="display" style="width:100%">
        ...
        @foreach($ds->users()->get() as $dsuser)
            <tr>
                <td data-order="{{$dsuser->email}}" data-search="{{$dsuser->email}}" name="collaborator_email">{{$dsuser->email}}</td>
                <td data-order="{{$dsuser->name}}" data-search="{{$dsuser->name}}">{{$dsuser->name}}</td>
                <td data-order="{{$dsuser->datasets()->find($ds->id)->pivot->dsrole}}" data-search="{{$dsuser->datasets()->find($ds->id)->pivot->dsrole}}">
                    <input type="text" name="old_dsrole" class="inputastd" value="{{$dsuser->datasets()->find($ds->id)->pivot->dsrole}}" disabled>
                    <select name="edit_collaborator_dsrole_selector" class="hideme" disabled>
                        <option value="ADMIN">ADMIN</option>
                        <option value="COLLABORATOR">COLLABORATOR</option></option>
