        ...
        <div class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="visualiseDropdown">
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-3d.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json'))">3D
                    Viewer</a>
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-cluster.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json'))">Cluster</a>
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-journey.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json?line=route'))">Journey
                    Route</a>
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-journey.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json?line=time'))">Journey
                    Times</a>
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-timeline.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json?sort=start'))">Timeline</a>
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-werekata.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json'))">Werekata
                    Flight by Route</a>
                <a class="dropdown-item grab-hover"
                    onclick="window.open('{{ config('app.views_root_url') }}/collection-werekata.html?load=' + encodeURIComponent('{{ url('publiccollections') }}/{{ $collection->id }}/json?sort=start'))">Werekata
                    Flight by Time</a>
            </div>
        </div>
    @endif
        ...
        <div class="col-lg-4">
            <div class="table-responsive" style="overflow: unset">
                <table class="table table-bordered">
                    ...
                    <tr>
                        <th>Source URL</th>
                        <td id="source_url">{{ $collection->source_url }}</td>
                    </tr>
                    <tr>
                        <th>Linkback</th>
            ...
            <tbody>
                @foreach ($collection->datasets as $ds)
                    <tr id="row_id_{{ $ds->id }}">
                        ...
                        <td>{{ $ds->ownerName() }} @if ($ds->owner() == Auth::user()->id)
                                (You)
                            @endif
                        </td>
                        ...
                        <td>
                            <button name="remove_dataset_button" data-collection-id="{{ $collection->id }}"
                                id="remove_dataset_button_{{ $ds->id }}" type="Button">Remove</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
