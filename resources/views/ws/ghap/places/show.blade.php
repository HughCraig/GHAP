    ...
    <div>
        ...
        @if (isset($details))
            <form method="post" action="{{ url('places/download/') }}">
                ...
                <div>
                    <script>
                        var downloadurl = window.location.href.replace(/&?\??paging=[0-9]*/, '').replace(/&?\??page=[0-9]*/, '').replaceAll(
                            '+', '%20');
                    </script>

                    <a href="{{ route('index') }}#advancedsearch" class="btn btn-secondary tlcmgreen" role="button"
                        id="advancedsearch"
                    ...
                    <!-- Export/Download -->
                    <div class="dropdown">
                        <button class="btn btn-secondary dropdown-toggle tlcmgreen" type="button" id="downloadDropdown"
                    ...
                    <!-- Web Services Feed -->
                    <div class="dropdown">
                        <button class="btn btn-secondary dropdown-toggle tlcmgreen" type="button" id="wsfeedDropdown"
                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            WS Feed
                        </button>
                ...
                <div class="mt-4 mb-1">
                    <p>Note: Layers are contributed from many sources by many people or derived by computer
                        ...
                        Check the layer for details about the source. Absence in TLCMap does not indicate absence in
                        reality.
                        Use of TLCMap may inform heritage research but is not a substitute for established formal and legal
                        processes and consultation.</p>
                </div>
                ...
                @guest
                    <div class="mb-2 form-group row">
                        <div class="col-xs-4">
                            <div class="mb-3 p-3 w3-pale-red"><a href="{{ url('/login') }}">Log in</a> to save searches and
                                contribute layers.</div>
                        </div>
                    </div>
                @else
                    <div class="mb-2 form-group row">
                        <div class="col-xs-4">
                            ...
                            <div class="mt-3 mb-3 p-3 w3-pale-green" id="save_search_message" style="display:none">Successfully
                                added this search to your <a href="{{ url('/myprofile/mysearches') }}">saved searches</a>!
                            </div>
                        </div>
            ...
            <div id="datasettable_wrapper" class="dataTables_wrapper no-footer mb-2">
                ...
                <div id="datasettable_filter" class="dataTables_filter" style="padding-right: 20px;">
                    @if (app('request')->input('subquery'))
                        <script>
                            var cancelSearch = window.location.href.replace(/&?\??subquery=.*/, '');
                        </script>
                        <button class="btn btn-secondary" onclick="location.href=cancelSearch">Cancel Sub-query</button>
                    @else
                        <label>
                            Sub-query:<input id="subsearchInput" type="search" style="padding: 6px;" class=""
                                placeholder="Search within results..." aria-controls="datasettable">
                            <button type="button" class="btn btn-primary" id="subsearchButton" onclick="subsearch()"><i
