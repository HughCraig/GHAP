...
@section('content')
    ...
    <table id="collectionsTable" class="display" style="width:100%">
        ...
        <tbody>
            @foreach ($collections as $collectionIdx => $collection)
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="visualiseDropdown">
                                    <a class="dropdown-item grab-hover"
                                        onclick="window.open('{{ config('app.views_root_url') }}/collection-3d.html?load=' + encodeURIComponent('{{ url()->full() }}/{{ $collection->id }}/json'))">3D
                                        Viewer</a>
                                    <a class="dropdown-item grab-hover"
                                        onclick="window.open('{{ config('app.views_root_url') }}/collection-cluster.html?load=' + encodeURIComponent('{{ url()->full() }}/{{ $collection->id }}/json'))">Cluster</a>
                                    <a class="dropdown-item grab-hover"
                                        onclick="window.open('{{ config('app.views_root_url') }}/collection-journey.html?load=' + encodeURIComponent('{{ url()->full() }}/{{ $collection->id }}/json?line=route'))">Journey
                                        Route</a>
                                    <a class="dropdown-item grab-hover"
                                        onclick="window.open('{{ config('app.views_root_url') }}/collection-journey.html?load=' + encodeURIComponent('{{ url()->full() }}/{{ $collection->id }}/json?line=time'))">Journey
                                        Times</a>
                                    <a class="dropdown-item grab-hover"
                                        onclick="window.open('{{ config('app.views_root_url') }}/collection-timeline.html?load=' + encodeURIComponent('{{ url()->full() }}/{{ $collection->id }}/json?sort=start'))">Timeline</a>
                                    <a class="dropdown-item grab-hover"
