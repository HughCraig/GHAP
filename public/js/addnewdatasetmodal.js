 ...
 *
 */
function validateAddLayerRequestData(msgBanner) {
    // Validate the input.
    let isValid = true;
    msgBanner.clear();
    ...
    if ($("#longitudefrom").val() !== "") {
        if (!Validation.longitude($("#longitudefrom").val())) {
            isValid = false;
            msgBanner.error("From Longitude must be valid from -180 to 180");
        }
    }
    ...
    if ($("#created").val() !== "" && !Validation.date($("#created").val())) {
        isValid = false;
...
function getAddLayerRequestData() {
    ...
    formData.append("public", $("#public").children("option:selected").val());
